...
def maybe_upgrade_password(conn, usuario: str, raw_password: str, stored_password: str):
    if stored_password and not stored_password.startswith("pbkdf2_sha256$") and raw_password == stored_password:
        conn.execute(text("UPDATE usuarios SET password = :password WHERE usuario = :usuario"), {
            "usuario": usuario,
...
ROLE_PERMISSIONS = {
    "ADMIN": {"resumen", "clientes_registrar", "clientes_gestion", "clientes_bd", "creditos", "contratos_pendientes", "detalle", "pagos", "simulador", "borrar_clientes"},
    "ASESOR": {"resumen", "clientes_registrar", "clientes_gestion", "clientes_bd", "creditos", "contratos_pendientes", "detalle", "pagos", "simulador"},
    "CONSULTA": {"resumen", "clientes_bd", "detalle", "simulador"},
}


def can_access(feature: str) -> bool:
    rol = (st.session_state.get("rol") or "").upper()
    return feature in ROLE_PERMISSIONS.get(rol, set())


def current_actor():
    return {
        "usuario": st.session_state.get("usuario") or "sistema",
        "rol": st.session_state.get("rol") or "SISTEMA",
    }


def registrar_auditoria(conn, accion: str, entidad: str, entidad_id: str = "", detalle=None):
    actor = current_actor()
    conn.execute(text("""
        INSERT INTO audit_log (usuario, rol, accion, entidad, entidad_id, detalle, created_at)
        VALUES (:usuario, :rol, :accion, :entidad, :entidad_id, :detalle, :created_at)
    """), {
...
def registrar_evento_credito(conn, prestamo_id: str, etapa: str, detalle: str):
    actor = current_actor()
    conn.execute(text("""
        INSERT INTO credito_eventos (prestamo_id, etapa, detalle, usuario, rol, fecha_evento)
        VALUES (:prestamo_id, :etapa, :detalle, :usuario, :rol, :fecha_evento)
    """), {
...
def init_usuario_admin():
    with get_conn() as conn:
        ...
        """))
        admin_exists = conn.execute(text("SELECT COUNT(*) FROM usuarios WHERE usuario = 'admin'" )).scalar()
        if int(admin_exists or 0) == 0:
            conn.execute(text("""
                INSERT INTO usuarios (usuario, password, rol)
                VALUES (:usuario, :password, :rol)
            """), {"usuario": "admin", "password": hash_password("Sandra*123"), "rol": "ADMIN"})
        conn.commit()
...
if not st.session_state.auth and not token_aceptar:
    ...
    if st.button("Ingresar"):
        with get_conn() as conn:
            ...
            if user and verify_password(clave, user["password"]):
                maybe_upgrade_password(conn, usuario, clave, user["password"])
    ...
    try:
        ...
        if ok_mail:
            with get_conn() as conn:
                ...
                registrar_auditoria(conn, "ENVIAR", "CONTRATO", prestamo_row["id"], {"correo": prestamo_row.get("correo", "")})
                registrar_evento_credito(conn, prestamo_row["id"], "CONTRATO_ENVIADO", "Contrato enviado al cliente")
                conn.commit()
    ...
    if ok_mail:
        with get_conn() as conn:
            conn.execute(text("""
                ...
                WHERE id = :id
            """), {"fecha": datetime.now().isoformat(timespec='seconds'), "id": prestamo_row["id"]})
            registrar_auditoria(conn, "ENVIAR", "DESEMBOLSO", prestamo_row["id"], {"correo": prestamo_row.get("correo", "")})
            registrar_evento_credito(conn, prestamo_row["id"], "DESEMBOLSO_NOTIFICADO", "Correo de desembolso enviado")
            conn.commit()
    return ok_mail, err_mail
...
def guardar_cliente_db(data):
    with get_conn() as conn:
        ...
        """), data)
        registrar_auditoria(conn, "CREAR", "CLIENTE", data.get("cedula"), {"nombre": f"{data.get('nombres','')} {data.get('apellidos','')}".strip()})
        conn.commit()
def actualizar_cliente_db(cedula, data):
    with get_conn() as conn:
        conn.execute(text("""
            ...
            WHERE cedula=:cedula
        """), {**data, "cedula": cedula})
        registrar_auditoria(conn, "ACTUALIZAR", "CLIENTE", cedula, {"correo": data.get("correo","")})
        conn.commit()
def eliminar_cliente_db(cedula):
    with get_conn() as conn:
        ...
        registrar_auditoria(conn, "ELIMINAR", "CLIENTE", cedula, {})
        conn.commit()
    return True, None
...
def crear_credito_db(cliente_cedula, monto, cuotas, frecuencia, tipo, fecha_inicio=None):
    ...
    with get_conn() as conn:
        ...
        registrar_auditoria(conn, "CREAR", "PRESTAMO", prestamo_id, {"tipo": tipo, "frecuencia": frecuencia, "cliente_cedula": cliente_cedula})
        registrar_evento_credito(conn, prestamo_id, "CREADO", f"Crédito {tipo} registrado en estado Pendiente")
        conn.commit()
        prestamo_row = {
            "id": prestamo_id,
...
def aceptar_contrato_por_token(token):
    with get_conn() as conn:
        ...
        registrar_auditoria(conn, "ACEPTAR", "CONTRATO", prestamo["id"], {"token": token})
        registrar_evento_credito(conn, prestamo["id"], "CONTRATO_ACEPTADO", "Cliente aceptó contrato y crédito pasó a Activo")
        conn.commit()
...
def registrar_pago_cuota(prestamo_id, fecha_pago):
    with get_conn() as conn:
        ...
        registrar_auditoria(conn, "REGISTRAR", "PAGO", prestamo_id, {"cuota": nro_cuota, "valor": float(valor_pago)})
        registrar_evento_credito(conn, prestamo_id, "PAGO_CUOTA", f"Pago registrado para cuota #{nro_cuota} por {float(valor_pago):.2f}")
        conn.commit()
...
def registrar_abono_capital(prestamo_id, fecha_pago, valor_abono):
    ...
    with get_conn() as conn:
        ...
        actualizar_estado_prestamo(conn, prestamo_id)
        registrar_auditoria(conn, "REGISTRAR", "ABONO_CAPITAL", prestamo_id, {"valor": float(valor_abono), "nueva_cuota": float(nueva_cuota)})
        registrar_evento_credito(conn, prestamo_id, "ABONO_CAPITAL", f"Abono a capital por {float(valor_abono):.2f}. Nueva cuota {float(nueva_cuota):.2f}")
        conn.commit()
        cliente = obtener_datos_cliente(conn, prestamo_db["cliente_cedula"])
    nombre_cliente = cliente[0] if cliente else "Cliente"
    correo_cliente = (cliente[1] or "").strip() if cliente else ""
...
with tab_clientes:
    ...
    client_tabs = []
    if can_access("clientes_registrar"):
        client_tabs.append(("registrar", "📝 Registrar cliente"))
    if can_access("clientes_gestion") or can_access("borrar_clientes"):
        client_tabs.append(("gestion", "🛠️ Gestión de clientes"))
    if can_access("clientes_bd"):
        client_tabs.append(("bd", "📋 BD clientes"))
...
with tab_creditos:
    ...
    credit_tabs = []
    if can_access("creditos"):
        credit_tabs.extend([("normal", "💳 Crédito normal"), ("express", "⚡ Crédito express")])
    if can_access("contratos_pendientes"):
        credit_tabs.append(("pendientes", "📨 Contratos pendientes"))
    credit_tab_objs = st.tabs([label for _, label in credit_tabs])
    credit_tab_map = {key: obj for (key, _), obj in zip(credit_tabs, credit_tab_objs)}
    ...
    def render_detalle_creditos(df_detalle: pd.DataFrame, empty_msg: str):
        ...
        for _, row in df_detalle.iterrows():
            with st.expander(f"💳 Préstamo {row['id']} — {row['cliente']}"):
                ...
                with get_conn() as conn:
                    eventos_credito = pd.read_sql(text("""
                        SELECT fecha_evento, etapa, detalle, usuario, rol
                        FROM credito_eventos
                        WHERE prestamo_id = :id
                        ORDER BY id DESC
                    """), conn, params={"id": row["id"]})
                t1, t2, t3 = st.tabs(["📅 Cuotas del crédito", "💸 Movimientos registrados", "🕘 Timeline"])
                with t1:
                ...
                with t3:
                    if eventos_credito.empty:
                        st.info("Sin eventos registrados para este crédito.")
                    else:
                        eventos_credito = eventos_credito.rename(columns={
                            "fecha_evento": "Fecha",
                            "etapa": "Etapa",
                            "detalle": "Detalle",
                            "usuario": "Usuario",
                        ...
                        st.dataframe(eventos_credito, use_container_width=True, hide_index=True)
