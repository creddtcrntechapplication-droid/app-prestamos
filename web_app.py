...
# ==========================
# 🛠️ UTILIDADES Y FUNCIONES DE APOYO
# ==========================
...
def enviar_correo_pago(destino, asunto, cuerpo, attachment_bytes=None, attachment_name=None):
    """
    Función síncrona para envío de correos. 
    Asegura que el correo salga antes de que Streamlit refresque la página.
    """
    try:
        msg = EmailMessage()
        msg["From"] = SMTP_USER
...
def generar_recibo_pdf(prestamo_id, cliente, monto_credito, fecha_pago, valor_pagado):
    """Crea el PDF profesional con el diseño de CREDDT."""
    
    # Importaciones necesarias dentro de la función para asegurar disponibilidad
    ...
    # --- MARCA + SLOGAN ---
    estilo_marca = ParagraphStyle('Marca', alignment=TA_CENTER, fontSize=14, spaceAfter=4)
    estilo_slogan = ParagraphStyle('Slogan', alignment=TA_CENTER, fontSize=10, textColor=gris, spaceAfter=20)
    ...
    # --- TÍTULO ---
    estilo_titulo = ParagraphStyle('Titulo', alignment=TA_CENTER, fontSize=18, textColor=azul, spaceAfter=25)
    elementos.append(Paragraph("RECIBO DE PAGO", estilo_titulo))

    # --- DATOS ---
    estilo_label = ParagraphStyle('Label', fontSize=11, textColor=gris, spaceAfter=6)
    estilo_valor = ParagraphStyle('Valor', fontSize=12, spaceAfter=12)
    ...
    estilo_pago = ParagraphStyle('PagoGrande', alignment=TA_CENTER, fontSize=28, textColor=azul, spaceAfter=5)
    
    elementos.append(Paragraph("VALOR PAGADO", estilo_label))
    ...
    estilo_footer = ParagraphStyle('Footer', alignment=TA_CENTER, fontSize=9, textColor=gris)
    elementos.append(Paragraph("Este documento es un comprobante oficial generado automáticamente.", estilo_footer))
    elementos.append(Paragraph("CREDDT CRNTECH APPLICATION • Cali - Colombia • creddtcrntechapplication@gmail.com", estilo_footer))
        ...
        with st.spinner("⏳ Aplicando pago, por favor espera..."):
            ...
            try:
                ...
                if correo_cliente:
                    ...
                    # Enviamos el correo de forma directa y síncrona
                    correo_ok = enviar_correo_pago(
                        correo_cliente,
                    ...
                    if os.path.exists(recibo_pdf):
                        os.remove(recibo_pdf)
