...
# CONEXIÓN A SUPABASE
# ==========================
# Tu secret en secrets.toml:
DATABASE_URL = "postgresql://postgres:t%21U8eLJ%2CUy%3Ffww6@db.hexwfdslcjwstzzfjxym.supabase.co:5432/postgres"

DATABASE_URL = st.secrets["DATABASE_URL"]

try:
    # Con SSL obligatorio para Supabase
...
try:
    with get_conn() as conn:
        prueba = conn.execute("SELECT 1").fetchone()
...
with tab_resumen:
    ...
    with get_conn() as conn:
        ...
        """, conn, params=(inicio.strftime("%Y-%m-%d"), fin.strftime("%Y-%m-%d")))
        ...
        with st.spinner("⏳ Aplicando pago, por favor espera..."):
            ...
            try:
                with get_conn() as conn:
                    ...
                    prestamo_db = cur.execute("""
                        SELECT cliente_cedula, monto_original
                        FROM prestamos
                        WHERE id=?
                    """, (prestamo.id,)).fetchone()
                    ...
                    for id_cuota, valor_cuota, nro in cuotas:
                        ...
                        nuevo_estado = "Pagada" if abono == saldo else "Parcial"
                ...
                cliente = ejecutar_sql(
                    "SELECT nombres || ' ' || apellidos, correo FROM clientes WHERE cedula=?",
                    (cliente_cedula,),
                    fetch=True
                )
